...
class Appr(object):
    ...
    def __init__(self,model,nepochs=100,sbatch=64,lr=0.05,lr_min=1e-7,lr_factor=3,lr_patience=5,clipgrad=10000,lamb=0.75,smax=400,args=None):
        self.model=model
    ...
    def train(self,t,xtrain,ytrain,xvalid,yvalid):
        ...
        try:
            ...
            train_loss_file_name = "train_loss_"+Path(self.args.output).stem
            train_loss_file_path = f"{Path(self.args.output).parent}/{train_loss_file_name}.csv"
            ...
            for e in range(self.nepochs):
                ...
                train_loss_df = pd.concat([train_loss_df,pd.DataFrame({"Task":t,"Epochs":e,"Train_loss":round(train_loss,6),
                                 "Train_acc":round(100*train_acc,6),
                                 "Avg_train_reg":round(train_avg_reg,6),
                ...
                if valid_loss<best_loss:
                    ...
                    if patience<=0:
                        ...
                        self.optimizer=self._get_optimizer(lr)
                print()
            train_loss_df.to_csv(train_loss_file_path,index=False)
